...
@section('content')
    ...
    @forelse($category->articles as $article)
        <div class="card mt-4" style="font-size: 1.1em">
            <h3 class="card-header badge-dark">{{$article->name}}</h3>
            <p class="card-body badge-light">{{$article->body}}</p>
            <small class="card-footer badge-secondary">Author: {{$article->author_id}} Created
                on: {{date("d-m-Y",strtotime($article->created_at))}}</small>
        </div>
    @empty
        <p class="text-danger">There are no articles</p>
    @endforelse
@endsection
